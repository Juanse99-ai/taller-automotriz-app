    ...
    <div className="card__h">
      <h3 style={{ margin: 0, flex: 1, minWidth: 0 }}>
        <button type="button" className="rep-toggle" aria-expanded={!colapso[id]} onClick={() => toggleColapso(id)}
          ...
          <svg className="rep-chevron" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
      ...
      <div className="hd-head rep-head">
        ...
        <div className="hd-head__right">
          ...
          <button type="button" className={`rep-rango${verRango ? ' on' : ''}`} onClick={() => setVerRango(v => !v)}
            ...
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="17" rx="2" /><path d="M8 2v4M16 2v4M3 10h18" /></svg>
          ...
          <button type="button" className="rep-ico" onClick={() => exportarResumen().catch(e => notify?.(`No se pudo generar el PDF: ${e?.message || e}`, 'error'))} disabled={sinDatos}
          ...
          <Button variant="primary" onClick={exportarCSV} disabled={sinDatos}
            ...
            icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3v12M7 10l5 5 5-5M4 19h16" /></svg>}>
            Exportar CSV
          </Button>
        </div>
      </div>
              ...
              {stats.porTecnico.map((t,i)=>{
                ...
                return (
                  <tr key={t.id}>
                    ...
                    <td className="c-mono c-right" data-label="Mano de obra" style={{fontWeight:400,color:'var(--text-3)'}}>{fmt(t.facturado)}</td>
                    <td className="td-bar">
