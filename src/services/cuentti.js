...
import { costoDeProducto, margenSobreVenta } from '../utils/costos'
    ...
    for (const e of arr) {
      ...
      if (id != null) {
        map.set(String(id), {
          ...
          costoBase: costoDeProducto(e), // Tier 2: costo si el endpoint de existencias lo trae
        })
      }
    }
    return map
...
// utilidadPct = MARGEN sobre la venta (utils/costos.js, el mismo del MCP de Cuentti).
...
export function derivarCosto(p) {
  ...
  p.utilidadPct = margenSobreVenta(precioBase, costoBase)
  return p
}
