...
export default async function handler(req, res) {
  ...
  if (req.query.verificarPagos) {
    ...
    try {
      ...
      // Lo ABONADO tal como lo dice Cuentti. Se devuelve aparte a proposito: el
      // portal lo mostraba restando (total de la app - pendiente de Cuentti), y
      // esas son dos fuentes distintas. Ademas, cada recibo queda copiado en
      // `pagos` (ver bajarPagosCuentti), que es lo que alimenta la cartera.
      const { marcados, saldos, abonos } = await sincronizarConCuentti(rows)
  ...
  // Sincronizar pagos con Cuentti desde la APP (con sesion): todas las facturas
  ...
  if (req.query.sincronizarPagos) {
    ...
    const filtro = cual === '1'
      ...
      : `id=eq.${encodeURIComponent(cual)}`
    try {
      const rows = await fetch(`${SUPABASE_URL}/rest/v1/trabajos?${filtro}&cuentti_id_transacion=not.is.null&select=id,cuentti_id_transacion,total&limit=80`,
        { headers: SUPABASE_HEAD }).then(r => (r.ok ? r.json() : []))
      const r = await sincronizarConCuentti(rows)
      res.status(200).json({ ok: true, ...r })
    } catch (e) {
      res.status(200).json({ ok: false, error: e.message, marcados: [], saldos: {}, abonos: {}, nuevos: 0, revisadas: 0 })
    }
    return
  }
  ...
  const TABLAS_SOLO_ADMIN = ['liquidados', 'liquidacion_historial', 'movimientos_tecnicos', 'prestamos_movimientos', 'gastos', 'gastos_registrados', 'compras_registradas']
  ...
  const ALLOWED_TABLES = [
    ...
    'compras_registradas',       // compras a proveedores registradas con Claude: el flujo de caja las cuenta
  ]
  ...
  if ((table === 'gastos_registrados' || table === 'compras_registradas') && req.method !== 'GET') { res.status(403).json({ error: 'La bitácora de Cuentti es de solo lectura' }); return }
