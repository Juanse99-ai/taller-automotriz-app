...
let clients = JSON.parse(localStorage.getItem('taller-clients') || '[]');
let inventory = JSON.parse(localStorage.getItem('taller-inventory') || '[]');
let selectedInventoryItems = []; // Para repuestos seleccionados en el formulario
...
function toggleSidebar() {
    ...
    if (header) {
        ...
        header.style.left = isCollapsed ? '60px' : '280px';
    }
}
...
const pageTitles = {
    ...
    'jobs': { title: 'Gestión de Trabajos', subtitle: 'Administra las órdenes de trabajo' },
    'mechanics': { title: 'Gestión de Mecánicos', subtitle: 'Administra el personal técnico' },
    'finances': { title: 'Finanzas', subtitle: 'Control financiero del taller' },
    'settlement': { title: 'Liquidación', subtitle: 'Gestión de comisiones' },
    'reception': { title: 'Recepción', subtitle: 'Recepción de vehículos' },
    'quotes': { title: 'Cotizaciones', subtitle: 'Gestión de cotizaciones' },
    'calendar': { title: 'Calendario', subtitle: 'Agenda de citas y trabajos' },
    'vehicle-history': { title: 'Historial de Vehículos', subtitle: 'Historial de servicios por vehículo' },
    'notifications': { title: 'Notificaciones', subtitle: 'Alertas y notificaciones' },
    'reports': { title: 'Reportes', subtitle: 'Reportes y estadísticas' }
};
...
function showSection(sectionName) {
    ...
    if (targetSection) {
        ...
        console.warn('Sección no encontrada: section-' + sectionName);
    }
    ...
    const pageInfo = pageTitles[sectionName];
    if (pageInfo) {
...
function loadSampleData() {
    ...
    // Datos de clientes para búsqueda
    let clients = [
        ...
        { id: 6, name: 'Patricia López', cedula: '33445566', phone: '3002223344', email: 'patricia.lopez@email.com' },
        { id: 7, name: 'Fernando Castro', cedula: '77889900', phone: '3008889900', email: 'fernando.castro@email.com' },
        { id: 8, name: 'Sandra Moreno', cedula: '44556677', phone: '3006667788', email: 'sandra.moreno@email.com' }
    ];
    localStorage.setItem('taller-clients', JSON.stringify(clients));

    // Inventario de repuestos para búsqueda POS con códigos de barras
    let inventory = [
        { id: 1, code: 'FO001', barcode: '7891234567890', name: 'Filtro de aceite', brand: 'Mann', price: 25000, stock: 15, category: 'Filtros', description: 'Filtro aceite motor', minStock: 5 },
        { id: 2, code: 'FA001', barcode: '7891234567891', name: 'Filtro de aire', brand: 'Mann', price: 18000, stock: 12, category: 'Filtros', description: 'Filtro aire motor', minStock: 3 },
        { id: 3, code: 'CO001', barcode: '7891234567892', name: 'Correa de distribución', brand: 'Gates', price: 85000, stock: 8, category: 'Motor', description: 'Correa 134 dientes', minStock: 2 },
        { id: 4, code: 'PA001', barcode: '7891234567893', name: 'Pastillas de freno', brand: 'Brembo', price: 35000, stock: 20, category: 'Frenos', description: 'Pastillas delanteras', minStock: 10 },
        { id: 5, code: 'DI001', barcode: '7891234567894', name: 'Disco de freno', brand: 'Brembo', price: 80000, stock: 10, category: 'Frenos', description: 'Disco ventilación', minStock: 5 },
        { id: 6, code: 'LI001', barcode: '7891234567895', name: 'Aceite motor 5W30', brand: 'Mobil', price: 45000, stock: 25, category: 'Lubricantes', description: 'Aceite sintético 4L', minStock: 10 },
        { id: 7, code: 'BA001', barcode: '7891234567896', name: 'Batería 12V 60Ah', brand: 'LTH', price: 180000, stock: 5, category: 'Eléctrico', description: 'Batería libre mantenimiento', minStock: 2 },
        { id: 8, code: 'BO001', barcode: '7891234567897', name: 'Bomba de agua', brand: 'Saleri', price: 120000, stock: 6, category: 'Motor', description: 'Bomba agua completa', minStock: 2 },
        { id: 9, code: 'SP001', barcode: '7891234567898', name: 'Sensor de presión', brand: 'Bosch', price: 65000, stock: 3, category: 'Eléctrico', description: 'Sensor presión aceite', minStock: 1 },
        { id: 10, code: 'AM001', barcode: '7891234567899', name: 'Amortiguador', brand: 'Monroe', price: 95000, stock: 8, category: 'Suspensión', description: 'Amortiguador delantero', minStock: 4 },
        { id: 11, code: 'NE001', barcode: '7891234567900', name: 'Neumático 195/65R15', brand: 'Michelin', price: 180000, stock: 12, category: 'Neumáticos', description: 'Neumático todo terreno', minStock: 8 },
        { id: 12, code: 'CA001', barcode: '7891234567901', name: 'Cables de bujía', brand: 'NGK', price: 35000, stock: 15, category: 'Eléctrico', description: 'Juego cables bujía', minStock: 5 },
        { id: 13, code: 'AB001', barcode: '7891234567902', name: 'Abrehuja', brand: 'Stanley', price: 15000, stock: 20, category: 'Herramientas', description: 'Abrehuja universal', minStock: 5 },
        { id: 14, code: 'CL001', barcode: '7891234567903', name: 'Clavija de contacto', brand: 'Bosch', price: 28000, stock: 10, category: 'Eléctrico', description: 'Clavija arranque', minStock: 3 },
        { id: 15, code: 'FR001', barcode: '7891234567904', name: 'Radiador', brand: 'Nissens', price: 220000, stock: 2, category: 'Motor', description: 'Radiador aluminio', minStock: 1 }
    ];
    localStorage.setItem('taller-inventory', JSON.stringify(inventory));
...
function settleJob(jobId) {
    ...
    // Verificar si tiene técnicos adicionales para trabajos conjuntos
    const mainMechanic = mechanics.find(m => m.id == job.mechanicId);
    const additionalMechanics = [];
    
    // Buscar mecánicos adicionales en el trabajo
    if (job.additionalMechanics) {
        job.additionalMechanics.forEach(mechanicId => {
            const mechanic = mechanics.find(m => m.id == mechanicId);
            if (mechanic) {
    ...
    // Combinar mecánico principal con adicionales
    const allTechnicians = [mainMechanic, ...additionalMechanics];
    const activeTechnicians = allTechnicians.filter(m => 
        m && (m.name === 'Pedro Barraza' || m.name === 'Víctor Padilla')
    );
    
    // Verificar que al menos uno de Pedro o Víctor esté involucrado
    if (activeTechnicians.length === 0) {
        alert('Este trabajo no involucra a Pedro o Víctor para la liquidación del 20%');
        return;
    }
    
    // Mostrar opciones de liquidación
    const options = `
LIQUIDAR TRABAJO: ${job.plate}
====================================

Técnicos involucrados: ${allTechnicians.map(m => m.name).join(', ')}
Trabajo: ${job.description}
Mano de Obra: ${formatCurrency(job.laborCost || 0)}

Selecciona el tipo de liquidación:
1. Liquidación completa (todos los técnicos)
2. Solo Pedro Barraza (20%)
3. Solo Víctor Padilla (20%)
4. Pedro + Víctor (20% cada uno)
5. Liquidación personalizada

Ingresa el número de opción (1-5):`;
    ...
    switch (choice) {
        case '1': // Liquidación completa
            const totalShare = (job.laborCost || 0) * 0.40;
            const individualShare = totalShare / 3; // Dividir entre 3
            techniciansToPay = allTechnicians;
            settlementDetails = `40% Total: ${formatCurrency(totalShare)}\nDividido entre 3: ${formatCurrency(individualShare)} c/u`;
            break;
            
        case '2': // Solo Pedro
            const pedroShare = (job.laborCost || 0) * 0.20;
            techniciansToPay = activeTechnicians.filter(m => m.name === 'Pedro Barraza');
            settlementDetails = `20% Solo Pedro: ${formatCurrency(pedroShare)}`;
            break;
            
        case '3': // Solo Víctor
            const victorShare = (job.laborCost || 0) * 0.20;
            techniciansToPay = activeTechnicians.filter(m => m.name === 'Víctor Padilla');
            settlementDetails = `20% Solo Víctor: ${formatCurrency(victorShare)}`;
            break;
            
        case '4': // Pedro + Víctor
            const jointShare = (job.laborCost || 0) * 0.20; // 20% cada uno
            techniciansToPay = activeTechnicians.filter(m => 
                m.name === 'Pedro Barraza' || m.name === 'Víctor Padilla'
            );
            settlementDetails = `20% cada uno (40% total): ${formatCurrency(jointShare)} c/u`;
            break;
            
        case '5': // Personalizado
            const customPercent = prompt('Ingresa el porcentaje para el técnico (ej: 15, 20, 25):');
            if (!customPercent || isNaN(customPercent)) {
                alert('Porcentaje inválido');
                return;
            }
            const customShare = (job.laborCost || 0) * (customPercent / 100);
            techniciansToPay = activeTechnicians;
            settlementDetails = `${customPercent}%: ${formatCurrency(customShare)}`;
            break;
    }
    
    const confirmMessage = `
CONFIRMAR LIQUIDACIÓN: ${job.plate}
====================================
${settlementDetails}
...
¿Confirmar liquidación?`;
    ...
    if (confirm(confirmMessage)) {
        ...
        job.settled = true;
        job.settledDate = new Date().toISOString().split('T')[0];
        job.settledAmount = techniciansToPay.length > 0 ? techniciansToPay.reduce((sum, tech) => {
            const techShare = (job.laborCost || 0) * 0.20; // 20% base
            return sum + techShare;
        }, 0) : 0;
        job.settledTechnicians = techniciansToPay.map(m => m.id);
        job.settlementType = choice;
